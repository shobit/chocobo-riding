...
class Simulation {
	 ...
	 * 
	 * @param $race object Model_Race
	 */
	public static function run($race) 
	{ 
		// ------
		//
		...
		$nbr_chocobos = 0;
		$chocobos = array();
		foreach ($race->chocobos->find_all() as $chocobo)
		{
		...
		while ($nbr_chocobos_arrived < $nbr_chocobos) #tant que tous les chocobos ne sont pas arrivés
		{
			...
			foreach ($chocobos as &$chocobo)
			{
				...
				if ($chocobo['distance_cumul'] >= $race->circuit->length)
				{
					...
					$position = Kohana::message('races', "pos$nbr_chocobos_arrived");
