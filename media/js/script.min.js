(function(a){a.fn.autoResize=function(j){var b=a.extend({onResize:function(){},animate:true,animateDuration:150,animateCallback:function(){},extraSpace:20,limit:1000},j);this.filter('textarea').each(function(){var c=a(this).css({resize:'none','overflow-y':'hidden'}),k=c.height(),f=(function(){var l=['height','width','lineHeight','textDecoration','letterSpacing'],h={};a.each(l,function(d,e){h[e]=c.css(e)});return c.clone().removeAttr('id').removeAttr('name').css({position:'absolute',top:0,left:-9999}).css(h).attr('tabIndex','-1').insertBefore(c)})(),i=null,g=function(){f.height(0).val(a(this).val()).scrollTop(10000);var d=Math.max(f.scrollTop(),k)+b.extraSpace,e=a(this).add(f);if(i===d){return}i=d;if(d>=b.limit){a(this).css('overflow-y','');return}b.onResize.call(this);b.animate&&c.css('display')==='block'?e.stop().animate({height:d},b.animateDuration,b.animateCallback):e.height(d)};c.unbind('.dynSiz').bind('keyup.dynSiz',g).bind('keydown.dynSiz',g).bind('change.dynSiz',g)});return this}})(jQuery);$('textarea#content').autoResize({onResize:function(){$(this).css({opacity:0.8});},animateCallback:function(){$(this).css({opacity:1});},animateDuration:300,extraSpace:40});$(document).ready(function()
{hide_all();$('.informations').show();$('#span_informations').css('font-weight',"bold");function hide_all(){$('.informations').hide();$('.competences').hide();$('.equipment').hide();$('.jobs').hide();}
function destrong_all(){$('#span_informations').css('font-weight','normal');$('#span_competences').css('font-weight','normal');$('#span_equipment').css('font-weight','normal');$('#span_jobs').css('font-weight','normal');}
$('#link_informations').click(function(){hide_all();$('.informations').show();destrong_all();$('#span_informations').css('font-weight',"bold");return false});$('#link_competences').click(function(){hide_all();$('.competences').show();destrong_all();$('#span_competences').css('font-weight',"bold");return false});$('#link_equipment').click(function(){hide_all();$('.equipment').show();destrong_all();$('#span_equipment').css('font-weight',"bold");return false});$('#link_jobs').click(function(){hide_all();$('.jobs').show();destrong_all();$('#span_jobs').css('font-weight',"bold");return false});$('.points a').click(function(){var url=$(this).attr("href").split('/');var max=url.length;var value=url[max-1];var apt=parseInt($('#'+value).text())+1;$('#points span').html('<img src="../../images/theme/loading.gif" />');$('.points').hide();$.ajax({url:"../../chocobo/aptitude_up/"+value,type:"GET",dataType:"json",success:function(result)
{if(result>0)
{$('#points span').html(result);$('#'+value).html(apt);$('.points').show();}
else
{$('#points').hide();}}});return false});$('#about').hide();$('.info').hide();$('.competences .line_tab .line .value small').hide();$('.competences .label').hover(function(){$('#about').show();$(this).parent().find('small').show();$('#about div').html($(this).next().html());return false},function(){$('#about').hide();$(this).parent().find('small').hide();return false;});});function decompte(id,tps,termine,showtitle)
{if(tps>=0){if(tps>=60*60*24){jours=Math.floor(tps/(60*60*24));jours="["+jours+"] ";}else{jours="";}
if(tps>=60*60){heures=Math.floor(tps/(60*60))%24;if(heures<10){heures='0'+heures;}
heures+=":";}else{heures="";}
if(tps>=60){minutes=Math.floor(tps/60)%60;if(minutes<10){minutes='0'+minutes;}
minutes+=":";}else{minutes="00:";}
if(tps>=0){secondes=tps%60;if(secondes<10){secondes='0'+secondes;}}
$('#'+id).html(jours+heures+minutes+secondes);if(showtitle&&!$.browser.msie)$('title').html(jours+heures+minutes+secondes+' | Chocobo Riding {BETA} |::');tps--;setTimeout(function(){decompte(id,tps,termine,showtitle)},1000);}else{$('#'+id).html(termine);if(showtitle&&!$.browser.msie)$('title').html(termine+' ::| Chocobo Riding {BETA} |::');}}
$(document).ready(function()
{$('.interests a').click(function()
{var url=$(this).attr("href").split('/');var max=url.length;var id=url[max-2];var value=url[max-1];$('#post'+id+' .interests .somme').html('<img src="../../images/theme/loading.gif" />');$.ajax({url:"../../post/interest/"+id+"/"+value,type:"GET",dataType:"json",success:function(result)
{$('#post'+id+' .interests .somme').text(result);if(result>0)$('#post'+id+' .interests .somme').prepend("+");if(value=="-1")
{$('#post'+id+' .interests .minus').addClass("red");$('#post'+id+' .interests .plus').removeClass("green");$('#post'+id+' .interests .delete').removeClass("none");$('#post'+id+' .interests .delete').addClass("grey");}
else if(value=="1")
{$('#post'+id+' .interests .plus').addClass("green");$('#post'+id+' .interests .minus').removeClass("red");$('#post'+id+' .interests .delete').removeClass("none");$('#post'+id+' .interests .delete').addClass("grey");}
else if(value=="0")
{$('#post'+id+' .interests .minus').removeClass("red");$('#post'+id+' .interests .plus').removeClass("green");$('#post'+id+' .interests .delete').removeClass("grey");$('#post'+id+' .interests .delete').addClass("none");}
result=parseInt(result);if(result<0)
{$('#post'+id+' .interests .somme').removeClass("green");$('#post'+id+' .interests .somme').addClass("red");}
else if(result>0)
{$('#post'+id+' .interests .somme').removeClass("red");$('#post'+id+' .interests .somme').addClass("green");}
else
{$('#post'+id+' .interests .somme').removeClass("red");$('#post'+id+' .interests .somme').removeClass("green");}}});return false});});function openShoutbox(){window.open('http://chocobo-riding.com/shoutbox','_blank','toolbar=0, location=0, directories=0, status=0, scrollbars=0, resizable=0, copyhistory=0, menuBar=0, width=400, height=600');}
$(document).ready(function()
{$('.locked').hide();$('#minimal_view').hide();$('#maximal_view').hide();$('#maximal_view').show();$('#minimal_view').click(function(){$('.locked').hide("slow");$('#minimal_view').hide();$('#maximal_view').show();return false;});$('#maximal_view').click(function(){$('.locked').show("slow");$('#maximal_view').hide();$('#minimal_view').show();return false;});});$(document).ready(function()
{var id=$('#wave .form input[name=id]').val();refreshWave();$('#wave .form').submit(function()
{$('.loading').html('<img src="../../images/theme/loading.gif" />');var content=$('#wave .form input[name=content]').val();$.post("../../wave/add/"+id,{'content':content},function(){$('#wave .form input[name=content]').val("");refreshWave();});return false;});function refreshWave(){$('.loading').html('<img src="../../images/theme/loading.gif" />');$.ajax({url:"../../wave/view/"+id,type:"GET",dataType:"html",success:function(result){if(result!="")$('#wave .wave_content').html(result);$('#wave .loading').html("");}});setTimeout(refreshWave,60000);}});;(function($){var bindIndex=0;$.fn.extend({bubbletip:function(tip,options){if($('table.bubbletip #'+$(tip).get(0).id).length>0){return this;}
